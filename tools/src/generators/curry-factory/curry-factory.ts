...
export async function curryFactoryGenerator(
  tree: Tree,
  options: CurryFactoryGeneratorSchema
) {
  const libName = names(options.name);
  const fileName = names(options.fileName);
  const promptFile = options.promptFile || '';
  const system =
    'You are a helpful AI code assistant that specializes in generating curried factories using the provided template and the user instructions.';

  // Path to template file
  const templateFile = 'tools/src/generators/curry-factory/factory.template.ts';
...
`,
  ...
  const outputFile = joinPathFragments(
    projectRoot,
    `src/lib/${fileName.fileName}.factory.ts`
  );
...
export default curryFactoryGenerator;
