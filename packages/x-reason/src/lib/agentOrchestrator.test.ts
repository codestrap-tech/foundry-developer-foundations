...
type ExecState = Extract<
  SMState,
  { id: string; task: string; includesLogic: boolean }
>;
...
describe('buildStateMachineWithTasks_NoScheduler', () => {
  test('Top-Down chain preserves original planner tasks and serial wiring (no parallel flags)', () => {
    ...
    const edges = Object.fromEntries(
      nodeOrder.map((id, i) => [
        id,
        i < nodeOrder.length - 1 ? [nodeOrder[i + 1]] : [],
      ]),
    );
    ...
    const partition: PartitionResult = {
      buRegions: [],
      tdChains: [
        { id: 'td_chain_1', path: [...nodeOrder] as unknown as string[] },
      ],
    };
  ...
  test('Bottom-Up region expands to parallel batch fan-out with joins and preserves tasks', () => {
    ...
    const fanTargets = fanouts.map((t) => ({
      target: t.target,
      parallel: t.parallel,
    }));
    ...
    const b2Fan = batch2Start.transitions.find(
      (t) => t.on === 'CONTINUE' && t.target === 'exec_D',
    );
    ...
    expect(join2.transitions.find((t) => t.on === 'CONTINUE')!.target).toBe(
      'success',
    );
