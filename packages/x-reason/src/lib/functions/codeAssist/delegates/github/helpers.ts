...
async function getGithubClient(): Promise<VersionControlService> {
  const makeGithubClient = container.get<() => Promise<VersionControlService>>(
    TYPES.VersionControlService,
  );
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        console.log(
          `Attempt ${attempt} failed, refreshing token and retrying in ${delayMs}ms...`,
        );
        await sleep(delayMs);
      }
    }
...
export async function writeFileIfNotFoundLocally(file: string | undefined) {
  if (file && !fs.existsSync(file)) {
    ...
    try {
      ...
      result = await withGithubRetry((service) =>
        service.getFile({
...
export async function saveFileToGithub(file: string, content?: string) {
  ...
  try {
    ...
    const result = await withGithubRetry((service) =>
      service.getFile({
        owner: process.env.GITHUB_REPO_OWNER || '',
        repo: process.env.GITHUB_REPO_NAME || '',
        path: fileName,
