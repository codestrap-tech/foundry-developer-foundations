...
function VickieInner() {
  ...
  const [getTaskListToolExecuted, setGetTaskListToolExecuted] =
    useState<boolean>(false);
  const [getTaskListToolAttempts, setGetTaskListAttempts] = useState<number>(0);
  const [executeTaskListToolExecuted, setExecuteTaskListToolExecuted] =
    useState<boolean>(false);
  const [executeTaskListToolAttempts, setExecuteTaskListAttempts] =
    useState<number>(0);
  ...
  const conversation = useConversation({
    ...
    clientTools: {
      getTaskListTool: async (parameters: {
        query: string;
        userId: string;
        threadId?: string;
      }) => {
        console.log(
          `local getTaskListTool called with: ${JSON.stringify(parameters)}`,
        );
        ...
        console.log(
          `Got threadId: ${threadId} back from callAskVickie execution, starting polling`,
        );
      ...
      executeTaskList: async (parameters: {
        plan: string;
        threadId: string;
      }) => {
        ...
        console.log(
          `local executeTaskList for execution Id ${threadId} called with: ${taskListRef.current}`,
        );
...
${message.message}`,
      ...
      const intervalId = setInterval(async () => {
        if (attempts >= maxAttempts) {
          ...
          conversation.sendContextualUpdate(
            `Polling timed out after max attempts: ${maxAttempts} for threadID/executionId: ${executionId}. This means we've polled for a result for the max possible attempts but no result was returned. The execution has failed.`,
          );
          return conversation.sendUserMessage(
            `My job failed for threadID/executionId: ${executionId}. Please let me know why.`,
          );
        }
          ...
          switch (action) {
            case 'getTaskListTool':
              ...
              if (result?.taskList) {
                ...
                conversation.sendUserMessage(
                  `Please summarize the results of the get task list tool for thread/executionId: ${executionId} and get my approval before calling the execute task list tool.`,
                );
                clearInterval(intervalId);
            ...
            case 'executeTaskList':
              ...
              if (machine?.state) {
                ...
                conversation.sendUserMessage(
                  `Please summarize the results of the execute task list tool for thread/executionId: ${executionId}.`,
                );
                clearInterval(intervalId);
  ...
  const renderedMessages = useMemo(() => {
    return messages
      ...
      .filter(
        (line) =>
          line.trim() !== '' && line.includes('New message received on:'),
      )
      .map((line, idx) => {
    ...
    "
      ...
      <Card className="flex flex-col justify-between p-6">
        <div className="h-full flex flex-col">
          <div
            className={`mt-6 flex flex-col gap-4 ${!getTaskListToolExecuted ? 'hidden' : ''}`}
          >
            {/* Status Indicator */}
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium">Task Tool Executed:</span>
              <div
                className={`w-3 h-3 rounded-full ${getTaskListToolExecuted ? 'bg-green-500' : 'bg-red-500'}`}
              />
            </div>
          ...
          <div
            className={`mt-6 flex flex-col gap-4 ${!executeTaskListToolExecuted ? 'hidden' : ''}`}
          >
            {/* Status Indicator */}
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium">Execute Tool Running:</span>
              <div
                className={`w-3 h-3 rounded-full ${executeTaskListToolExecuted ? 'bg-green-500' : 'bg-red-500'}`}
              />
            </div>
            ...
            <div className="w-full">
              ...
              <div className="w-full h-2 bg-gray-200 rounded-full">
                <div
                  ...
                  style={{
                    width: `${(executeTaskListToolAttempts / 10) * 100}%`,
                  }}
                />
              </div>
            </div>
      ...
      {taskList && (
        <Card className="flex flex-col justify-between p-6 min-w-[22rem]">
          {' '}
          {/* give the card a height & flex layout */}
      ...
      <style jsx global>{`
        ...
        * {
        ...
        .orb {
          ...
          background:
            radial-gradient(
              color-mix(in srgb, var(--darker-color) 65%, transparent) -50%,
              transparent 50%
            ),
            radial-gradient(var(--color), var(--color)) no-repeat 50% 50% / 50%
              50%,
            ...
            conic-gradient(
                ...
                transparent 288deg,
                color-mix(in srgb, var(--lighter-color), transparent)
              )
              border-box,
            radial-gradient(
                farthest-corner at 50% 50%,
                transparent 50%,
                var(--darker-color) 80% 100%
              )
              border-box;

          background-blend-mode:
            normal, overlay, multiply, normal, normal, normal, normal;
          ...
          animation:
            10s linear infinite change-color,
            5s linear infinite orb;
        }
